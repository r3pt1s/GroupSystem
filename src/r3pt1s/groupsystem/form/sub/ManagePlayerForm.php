...
final class ManagePlayerForm extends MenuForm {
    ...
    public function __construct(string $username, private readonly PlayerGroup $currentGroup, string $message = "") {
        ...
        parent::__construct(
            Message::MANAGE_PLAYERS_UI_TITLE()->parse([$username]),
            $message, 
            [
                new MenuOption(Message::MANAGE_PLAYERS_UI_ADD_GROUP()),
            ...
            function(Player $player, int $data) use($username): void {
                if ($data == 0) {
                    ...
                    try {
                        ...
                        GroupSystem::getInstance()->getLogger()->logException($exception);
                    }
                } else if ($data == 4) {
                    $player->sendForm(new CustomForm(
                        ...
                        function(Player $player, CustomFormResponse $response) use($username): void {
                            ...
                            $time = Utils::convertStringToDateFormat($response->getString("time")) ?? null;
                            if ($permission !== "") {
                    ...
                    Session::get($username)->onLoad(function (PlayerGroup $group, array $groups, array $permissions) use($player, $username): void {
                        $player->sendForm(new MenuForm(

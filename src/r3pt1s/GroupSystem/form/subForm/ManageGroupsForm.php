            ...
            if ($data == 0) {
                $player->sendForm(new CustomForm(
                    ...
                    function(Player $player, CustomFormResponse $response): void {
                        ...
                        $displayName = $response->getString("nameTag");
                        $colorCode = $response->getString("nameTag");
                        $chatFormat = $response->getString("nameTag");
                        $permissions = explode(";", $response->getString("permissions"));

                        if ($name !== "") {
                            if (!GroupManager::getInstance()->isGroupExisting($name)) {
                                GroupManager::getInstance()->createGroup($group = new Group($name, $nameTag, $displayName, $colorCode, $chatFormat, $permissions));

...
class PlayerGroupManager {
    ...
    public function checkPlayer(string $username): bool {
        return file_exists(Configuration::getInstance()->getPlayersPath() . "{$username}.json");
    }
    ...
    public function getPlayerConfig(Player|GroupPlayer|string $username): Config {
        ...
        return new Config(Configuration::getInstance()->getPlayersPath() . "{$username}.json", 1);
    }
    
    public function getAllPlayersConfig(): array {
        $configs = [];
        foreach (scandir(Configuration::getInstance()->getPlayersPath()) as $file) {
            if ($file == "." || $file == "..") continue;
            if (pathinfo(Configuration::getInstance()->getPlayersPath() . $file, PATHINFO_EXTENSION) == "json") {

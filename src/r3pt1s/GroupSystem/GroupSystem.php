...
class GroupSystem extends PluginBase {
    ...
    private function loadPermissions() {
        ...
        if ($operator !== null) {
            DefaultPermissions::registerPermission(new Permission("groupsystem.group.command"), [$operator]);
        }
    }
    ...
    private function createDefaultMessages() {
        $messages = [
            ...
            "create_group_ui_colorcode" => "§7ColorCode of the Group §8(§7Example: §ePARAGRAPH+4 §8= §4DARK_RED§8)",
            "create_group_ui_chatformat" => "§7ChatFormat of the Group §8(§e{name} §8= §7Player§8, §e{msg} §8= §7Message§8)",
        ...
        if (!file_exists($this->getDataFolder() . "messages.yml")) {
            new Config($this->getDataFolder() . "messages.yml", 2, $messages);
        }
    }
